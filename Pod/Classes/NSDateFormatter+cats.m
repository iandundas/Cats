...
#import "NSDateFormatter+cats.h"


@implementation NSDateFormatter (cats)
...
+ (NSDateFormatter *)timeFormatterWithSecondsInstance {
    static NSDateFormatter *_timeFormatterWithSecondsInstance = nil;

    @synchronized (self) {
        if (_timeFormatterWithSecondsInstance == nil) {
            _timeFormatterWithSecondsInstance = [[self alloc] init];
            _timeFormatterWithSecondsInstance.locale= [NSLocale autoupdatingCurrentLocale];
            _timeFormatterWithSecondsInstance.dateFormat= @"HH:mm:ss";
        }
    }

    return _timeFormatterWithSecondsInstance;
}
